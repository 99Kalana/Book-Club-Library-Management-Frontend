...
import { MdEdit, MdDelete, MdArrowUpward, MdArrowDownward, MdEmail, MdPhone, MdPerson, MdCalendarToday } from "react-icons/md";
...
interface ReadersTableProps {
    ...
    sortColumn: keyof Reader | null;
    sortDirection: 'asc' | 'desc';
    onSort: (column: keyof Reader) => void;
}
...
const ReadersTable: React.FC<ReadersTableProps> = ({
    ...
    const renderSortIcon = (column: keyof Reader) => {
    ...
    const headers: { key: keyof Reader; label: string; sortable: boolean; icon?: React.ReactNode }[] = [
        { key: '_id', label: 'ID', sortable: true, icon: <span className="text-xs">#</span> },
        { key: 'name', label: 'Name', sortable: true, icon: <MdPerson className="w-4 h-4" /> },
        { key: 'email', label: 'Email', sortable: true, icon: <MdEmail className="w-4 h-4" /> },
        { key: 'phone', label: 'Phone', sortable: true, icon: <MdPhone className="w-4 h-4" /> },
        { key: 'registeredDate', label: 'Registered', sortable: true, icon: <MdCalendarToday className="w-4 h-4" /> },
    ];
                        ...
                        <tr>
                            <td colSpan={headers.length + 1} className='px-6 py-12 text-center'>
                                <div className="flex flex-col items-center space-y-4">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-medium text-gray-900 mb-1">No readers found</h3>
                                        <p className="text-gray-500">Try adjusting your search criteria or add a new reader.</p>
                                    </div>
                                </div>
                            </td>
                        </tr>
                                ...
                                <td className='px-6 py-4 whitespace-nowrap'>
                                    <div className='text-sm text-gray-700'>
                                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 group-hover:bg-blue-200 transition-colors">
                                                {new Date(reader.registeredDate).toLocaleDateString('en-US', {
                                                    year: 'numeric',
                                                    month: 'short',
                                                    day: 'numeric'
                                                })}
